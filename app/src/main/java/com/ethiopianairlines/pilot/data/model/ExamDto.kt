...
data class ExamDto(
    ...
    val category: String,       // e.g., "pilotTrainee" from backend
    val difficulty: String,     // e.g., "medium" from backend
    val durationMinutes: Int,
    val passingScore: Int,
    val totalQuestions: Int,
    val isActive: Boolean,
    val questions: List<QuestionResponseDto> = emptyList(), // List of DTOs for questions
...
//    val version: Int? = null    // Nullable if not always present or not needed
    ...
    fun toDomain(): DomainExam {
        // Determine domain category based on backend string
        val domainCategory = when (category.lowercase()) {
            ...
            "pilotexaminer" -> ExamCategory.PILOT_EXAMINER.toString()
                ...
                // For now, defaulting or logging
                println("Warning: Unknown category '$category' received for exam ID '$id'. Defaulting.")
                ExamCategory.PILOT_TRAINEE.toString() // Or throw IllegalArgumentException("Unknown category: $category")
            }
        }

        // Determine domain difficulty based on backend string
        val domainDifficulty = when (difficulty.lowercase()) {
            ...
            "hard" -> ExamDifficulty.HARD.toString()
                ...
                ExamDifficulty.MEDIUM.toString() // Or throw IllegalArgumentException("Unknown difficulty: $difficulty")
            }
        }
        ...
        return DomainExam(
            ...
            totalQuestions = this.totalQuestions, // Or questions.size if this DTO is the source of truth
            isActive = this.isActive,
            // Map each QuestionResponseDto to DomainQuestion
            // Assumes QuestionResponseDto has a toDomainModel() method as defined previously

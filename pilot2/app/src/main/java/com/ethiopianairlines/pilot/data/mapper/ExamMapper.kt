...
object ExamMapper {
    ...
    fun toDataModel(domainExam: DomainExam): DataExam {
        ...
        return DataExam(
            // id for DataExam is String? (nullable) now.
            // If creating a new exam, the backend should assign the ID, so we send null if id is blank/empty.
            id = domainExam.id.takeIf { it.isNotBlank() },
    ...
    fun toDomainModel(dataExam: DataExam): DomainExam {
        return DomainExam(
            ...
            category = when (dataExam.category?.lowercase()) {
                null -> ExamCategory.PILOT_TRAINEE.toString()
            ...
            difficulty = when (dataExam.difficulty?.lowercase()) {
                null -> ExamDifficulty.MEDIUM.toString()
            ...
            questionCount = 0, // Default value, not in list item DTO
